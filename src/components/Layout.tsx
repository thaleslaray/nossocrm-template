...
// Lazy load AI Assistant (componente pesado com SDK Gemini)
  ...
  return (
    <NavLink
      ...
      className={({ isActive, isPending }) => {
        // Keep highlight if this was the last clicked item (bridges Suspense gap)
        const wasJustClicked = clickedPath === to;

        // Special case for pipelines/boards alias
        const isActuallyActive = isActive || isPending || wasJustClicked ||
          (to === '/boards' && location.pathname === '/pipeline');

        return `flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium focus-visible-ring
    ${isActuallyActive
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  React.useEffect(() => {
    if (clickedPath) {
      ...
      const isNowActive = location.pathname === clickedPath ||
        (clickedPath === '/boards' && location.pathname === '/pipeline') ||
        (clickedPath === '/pipeline' && location.pathname === '/boards');
              ...
              return (
                <NavLink
                  ...
                  className={({ isActive, isPending }) => {
                    ...
                    const isActuallyActive = isActive || isPending || wasJustClicked ||
                      (item.to === '/boards' && location.pathname === '/pipeline');
