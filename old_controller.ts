...
export const useBoardsController = () => {
  ...
  const handleDrop = (e: React.DragEvent, stageId: string) => {
    ...
    if (dealId && activeBoard) {
      ...
      console.log('[DnD] Target stage:', targetStage?.label, 'linkedLifecycleStage:', targetStage?.linkedLifecycleStage);
      ...
      if (targetStage?.linkedLifecycleStage === 'OTHER') {
        ...
        moveDealMutation.mutate({
          ...
          board: activeBoard,
          lifecycleStages,
          tenantCompanyId: organizationId || profile?.organization_id || '',
        });
      }
    }
    setDraggingId(null);
  ...
  const handleCreateBoard = async (boardData: Omit<Board, 'id' | 'createdAt'>, order?: number) => {
    createBoardMutation.mutate({ board: boardData, order, organizationId: organizationId || profile?.organization_id }, {
      onSuccess: newBoard => {
        if (newBoard) {
